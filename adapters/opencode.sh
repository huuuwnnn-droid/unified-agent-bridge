...
for line in lines:
    ...
    if isinstance(text, str) and text:
        if event_type in {"text", "message", "assistant", "final", "response.completed"} or not event_type:
            last_text = text
...
if [[ -z "$TASK" ]]; then
  END_MS=$(now_ms)
  emit_json "opencode" "$TASK" "failed" "" "0" "0.0" "$((END_MS - START_MS))" "2" '["missing task"]'
  exit 0
fi

if [[ ! -d "$WORKDIR" ]]; then
  END_MS=$(now_ms)
  emit_json "opencode" "$TASK" "failed" "" "0" "0.0" "$((END_MS - START_MS))" "2" '["workdir not found"]'
  exit 0
fi

if ! command -v opencode >/dev/null 2>&1; then
  END_MS=$(now_ms)
  emit_json "opencode" "$TASK" "failed" "" "0" "0.0" "$((END_MS - START_MS))" "127" '["tool not installed"]'
  exit 0
fi
...
# --- Command ---
CMD=(opencode run "$TASK" --format json)
...
END_MS=$(now_ms)
emit_json "opencode" "$TASK" "$STATUS" "$PARSED_OUTPUT" "$TOKENS_USED" "$COST_USD" "$((END_MS - START_MS))" "$EXIT_CODE" "$ERRORS"
